# PM Customer Journey Analyzer

A browser-only checkout and customer journey funnel analyzer for Product Managers.

## What It Does

- Uploads CSV and supported PDF exports locally in the browser.
- Supports Adobe Analytics Step Conversion exports.
- Supports Adobe Analytics Fallout Funnel exports.
- Supports Augmeta weekly cart and checkout narrative PDFs.
- Supports simple `Funnel Step,Users` or `Funnel Step,Sessions` CSVs.
- Displays a report-style visual readout with metric blocks and trend panels.
- Keeps adding uploaded files into an AI Insights analysis set with per-file removal and clear-all controls.
- Detects largest dropoff and funnel health.
- Compares Adobe segment/date-range views against sensible baselines.
- Produces AI-style PM insights with improvement levers, executive context, watch items, and open hypotheses.
- Generates a copyable executive summary.

## Running Locally

Open `index.html` in a browser, or serve the repo folder with any static file server.

No backend, install step, or API key is required for the V0 prototype. Uploaded files are processed in browser memory only.
